...
abstract class OtelKotlinTestRule {
     ...
     */
    val config: io.opentelemetry.kotlin.framework.TestHarnessConfig =
        _root_ide_package_.io.opentelemetry.kotlin.framework.TestHarnessConfig()
    ...
    private val spanExporter =
        _root_ide_package_.io.opentelemetry.kotlin.framework.InMemorySpanExporter()
    private val logRecordExporter =
        _root_ide_package_.io.opentelemetry.kotlin.framework.InMemoryLogRecordExporter()
    ...
    protected val loggerProviderConfig: LoggerProviderConfigDsl.() -> Unit = {
        ...
        addLogRecordProcessor(
            _root_ide_package_.io.opentelemetry.kotlin.framework.InMemoryLogRecordProcessor(
                logRecordExporter
            )
        )
    ...
    protected val tracerProviderConfig: TracerProviderConfigDsl.() -> Unit = {
        config.attributes?.let { resource(config.schemaUrl, it) }
        addSpanProcessor(
            _root_ide_package_.io.opentelemetry.kotlin.framework.InMemorySpanProcessor(
                spanExporter
            )
        )
    ...
    fun assertLogRecords(
        ...
        if (goldenFileName != null) {
            _root_ide_package_.io.opentelemetry.kotlin.framework.compareGoldenFile(
                data,
                goldenFileName
            )
        }
