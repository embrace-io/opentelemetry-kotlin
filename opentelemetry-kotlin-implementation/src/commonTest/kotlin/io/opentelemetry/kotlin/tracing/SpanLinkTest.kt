...
internal class SpanLinkTest {
    ...
    fun setUp() {
        ...
        spanLimitConfig = SpanLimitConfig(
            attributeCountLimit = _root_ide_package_.io.opentelemetry.kotlin.tracing.fakeSpanLimitsConfig.attributeCountLimit,
            linkCountLimit = linkLimit,
            eventCountLimit = _root_ide_package_.io.opentelemetry.kotlin.tracing.fakeSpanLimitsConfig.eventCountLimit,
            attributeCountPerEventLimit = _root_ide_package_.io.opentelemetry.kotlin.tracing.fakeSpanLimitsConfig.attributeCountPerEventLimit,
            attributeCountPerLinkLimit = _root_ide_package_.io.opentelemetry.kotlin.tracing.fakeSpanLimitsConfig.attributeCountPerLinkLimit
        )
        tracer = TracerImpl(
            clock,
            processor,
    ...
    fun testSpanLinkAttributesLimit() {
        val span = tracer.createSpan("test", action = {
            addLink(FakeSpanContext()) {
                repeat(_root_ide_package_.io.opentelemetry.kotlin.tracing.fakeSpanLimitsConfig.attributeCountLimit + 1) {
                    setStringAttribute("foo$it", "bar")
                }
            }
        })
        val link = span.links.single()
        assertEquals(_root_ide_package_.io.opentelemetry.kotlin.tracing.fakeSpanLimitsConfig.attributeCountLimit, link.attributes.size)
    }
