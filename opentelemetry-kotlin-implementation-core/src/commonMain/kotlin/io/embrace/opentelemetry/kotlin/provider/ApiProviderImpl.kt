...
@ThreadSafe
public class ApiProviderImpl<T>(
    private val supplier: (key: InstrumentationScopeInfo) -> T
) {

    private val map = mutableMapOf<InstrumentationScopeInfo, T>()

    public fun getOrCreate(key: InstrumentationScopeInfo): T = sync(map) {
        map.getOrPut(key) {
            supplier(key)
        }
    }

    public fun createInstrumentationScopeInfo(
        name: String,
        version: String?,
        schemaUrl: String?,
